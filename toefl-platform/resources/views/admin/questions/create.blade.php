...
<div class="container mx-auto px-4 py-8">
    <div class="mb-6">
        ...
        <h1 class="text-2xl font-bold text-gray-800 mt-2">Create New Question</h1>
    </div>
        ...
        <div class="bg-white rounded-lg shadow p-6">
            ...
            <div class="mb-4">
                ...
                <textarea name="question_text" rows="4" required 
                          class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500">{{ old('question_text') }}</textarea>
            </div>
            ...
            <div class="mb-4">
                ...
                <textarea name="passage_text" rows="4" 
                          class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500">{{ old('passage_text') }}</textarea>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <input type="text" name="audio_url" value="{{ old('audio_url') }}" 
            ...
            <div class="mb-4">
                ...
                <textarea name="explanation" rows="3" 
                          class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500">{{ old('explanation') }}</textarea>
            </div>
        ...
        <div class="bg-white rounded-lg shadow p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div>
                    ...
                    <input type="number" name="preparation_time" value="{{ old('preparation_time') }}" min="0" 
                           class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Response Time (sec)</label>
                    <input type="number" name="response_time" value="{{ old('response_time') }}" min="0" 
                           class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Word Limit Min</label>
                    <input type="number" name="word_limit_min" value="{{ old('word_limit_min') }}" min="0" 
                           class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Word Limit Max</label>
                    <input type="number" name="word_limit_max" value="{{ old('word_limit_max') }}" min="0" 
                           class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500">
                </div>
            </div>
        </div>
                ...
                @if(old('options'))
                    @foreach(old('options') as $index => $option)
                        <div class="option-row flex gap-2 items-start">
                            <input type="checkbox" name="options[{{ $index }}][is_correct]" {{ $option['is_correct'] ?? false ? 'checked' : '' }} 
                                   class="mt-2" title="Is Correct">
                            <input type="text" name="options[{{ $index }}][option_text]" value="{{ $option['option_text'] ?? '' }}" 
                                   placeholder="Option text" required
                                   class="flex-1 border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500">
                            <input type="number" name="options[{{ $index }}][order_index]" value="{{ $option['order_index'] ?? $index }}" 
